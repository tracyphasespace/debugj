...
class StateMultivectorBase:
    ...
    def calculate_size(self) -> int:
        """Calculate the total memory size of this state multivector in bytes."""
        # Base implementation: 32 bytes for pose motor + 4*4 bytes for attributes
        return 32 + len(self.attributes) * 4
...
class OpticalState(StateMultivectorBase):
    ...
    def calculate_size(self) -> int:
        """Calculate the total memory size of this optical state in bytes."""
        # Optical state: 32 bytes pose motor + 16 bytes optical + 16 bytes material + 32 bytes spatial = 96 bytes
        return 96
