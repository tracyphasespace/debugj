...
- Optional: Numba 0.56+ for enhanced JIT compilation
...
# Create multivectors
a = alg.vector(e1=1, e2=2, e3=3)
b = alg.bivector(e12=1, e23=2, e31=3)

# Perform operations (first call compiles, subsequent calls are fast)
...
alg = Algebra(3, 0, 0, use_gpu=True)

# Batch operations for maximum GPU efficiency
batch_processor = alg.create_batch_processor(use_gpu=True, batch_size=1000)
results = batch_processor.geometric_product_batch(vector_list_a, vector_list_b)
```

### Performance Optimization Pattern
```python
# Pre-compile frequently used operations
scalar_template = alg.scalar(1.0)
vector_template = alg.vector(e1=1.0, e2=1.0, e3=1.0)

# Warm-up phase (eliminates first-call compilation overhead)
_ = scalar_template * vector_template
_ = vector_template * vector_template
_ = vector_template + vector_template

# Now all subsequent operations of these types are optimized
```
...
batch_processor = algebra.create_batch_processor(use_gpu=True, batch_size=1000)

# Example: Process 10,000 geometric products on GPU
import numpy as np
vectors_a = [algebra.vector(e1=np.random.rand(), e2=np.random.rand(), e3=np.random.rand()) 
             for _ in range(10000)]
vectors_b = [algebra.vector(e1=np.random.rand(), e2=np.random.rand(), e3=np.random.rand()) 
             for _ in range(10000)]

# GPU batch processing (10M+ ops/sec)
gpu_results = batch_processor.geometric_product_batch(vectors_a, vectors_b)

# Automatic memory management and CUDA stream optimization
with algebra.gpu_context() as gpu:
    # Operations within this context use GPU memory pools
    large_multivector = gpu.create_multivector_array(shape=(1000, 1000))
    processed = gpu.batch_operation(large_multivector, operation='geometric_product')
```

Advanced GPU Features:
```python
# Kernel fusion for complex operation chains
tape = algebra.create_tape_recorder(use_gpu=True)
with tape:
    result = ((a * b) + c) * (~d)  # Records operation graph
optimized_gpu_result = tape.execute_fused()  # Single GPU kernel execution

# Memory-efficient streaming for large datasets
stream_processor = algebra.create_stream_processor(gpu_streams=4)
for batch in large_dataset_iterator:
    stream_processor.queue_batch(batch, operation='geometric_product')
results = stream_processor.collect_results()
```
...
- NumPy 1.20+
- SymPy 1.9+
- Memory: 4GB RAM minimum, 8GB+ recommended for large computations

### Optional Dependencies for Enhanced Performance
- **Numba 0.56+**: Enhanced JIT compilation with LLVM backend
- **CUDA Toolkit 11.0+**: GPU acceleration support (NVIDIA GPUs only)
- **OpenMP**: Multi-threading support (usually included with compiler)
- **Intel MKL**: Optimized linear algebra operations

### Platform Support
- **Linux**: Full support including GPU acceleration
- **Windows**: Full support, GPU acceleration requires CUDA toolkit
- **macOS**: CPU-only support (Metal GPU support planned for future release)

### Performance Scaling Guidelines
- **Single-threaded**: Optimal for operations < 1000 elements
- **Multi-threaded**: Recommended for 1000-100,000 elements
- **GPU acceleration**: Most efficient for > 10,000 elements
- **Memory usage**: ~8 bytes per multivector coefficient + JIT cache overhead

8. Onboarding Instructions for AI Systems and Developers
...
**JIT Compilation Errors**: Ensure SymPy expressions are simplified before lambdify. Check for unsupported operations in the target backend (CPU/GPU).

**GPU Memory Issues**: Use batch processing with appropriate batch sizes. Monitor GPU memory usage with `algebra.gpu_memory_info()`.

**Performance Degradation**: Check JIT cache hit rates with `algebra.cache_statistics()`. Inconsistent multivector structures reduce cache efficiency.

**Threading Issues**: Ensure thread-safe multivector creation. Use `algebra.create_thread_local_context()` for multi-threaded applications.
